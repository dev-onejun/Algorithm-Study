...
5. **Persist the evaluation in the solution file.** After delivering the critique, append a triple-quoted `Review` block at the bottom of `2026/<Problem>.py`. Format: dated heading, verdict + asymptotic + stress numbers, bulleted issues, recurring patterns named (cross-reference the memory file `user_recurring_patterns`), and the optimum gap. Keep it scannable — the chat has the long-form, the file preserves the takeaway. See existing `2026/*.py` for the format.
